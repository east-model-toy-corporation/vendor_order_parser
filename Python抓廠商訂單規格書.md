...
1.  **以 AI 結果為準並進行價格驗證 (邏輯修正)**：當 AI 成功回傳資料時，程式將**直接以 AI 回傳的 `products` 陣列作為處理的唯一資料來源**，不再依賴本地的初步提取結果。接著，程式會逐一檢查此陣列中的每一筆商品，並嚴格執行價格驗證：**只有當 `起始進價` 和 `建議售價` 兩個欄位都存在且有值時**，該商品才會被保留並進入後續處理。這個步驟確保了 AI 提取的廣度與資料驗證的嚴謹性相結合。
2.  **欄位對映與品牌指派 (規則更新)**：
