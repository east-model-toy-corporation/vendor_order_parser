...
class App:
    def __init__(self, root):
        ...
        self.sheet_label = tk.Label(
            self.root, text="究極進化版資料夾的 Google Drive 網址:"
        )
        ...
        self.import_button = tk.Button(
            btn_frame, text="匯入檔案", command=self.import_files, height=2, width=12
        )
        ...
        self.log(
            "請先輸入您的 OpenAI API Key，使用「匯入檔案」選擇檔案，最後按「開始處理」並指定輸出檔名。"
        )
    ...
    def run_processing_thread(self):
        ...
        thread = threading.Thread(
            target=process_files_main,
            args=(self, api_key, self.input_files, self.output_file),
        )
        thread.daemon = True
        thread.start()
